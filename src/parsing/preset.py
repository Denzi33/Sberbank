...
month = day * 30
year = day * 365
last_day = datetime.datetime(2023, 5, 17)
uniq_subjs = [
...
data_features = [
               'time_dummie', 'year', 'month', 'day_of_month', 'hour', 'day_of_week',
...
def prep_forecast_data(
    ...
    main_df["subject_name"] = decoding(main_df[uniq_subjs[0]])
    ...
    forecast_data["lag_hour"] = create_lag_feature(forecast_data, hour)
    forecast_data["lag_day"] = create_lag_feature(forecast_data, day)
    forecast_data["lag_week"] = create_lag_feature(forecast_data, week)
    forecast_data["lag_month"] = create_lag_feature(forecast_data, month)
    forecast_data["lag_year"] = create_lag_feature(forecast_data, year)
